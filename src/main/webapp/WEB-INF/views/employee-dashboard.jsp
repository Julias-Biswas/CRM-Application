...
<title>Employee Dashboard - CRM</title>
...
<body>
	...
	<div class="admin_dashboard_section">
		<div class="container admin_dashboard">
			<h3 class="heading-three text-center">Welcome
				${session_emp.getName()}</h3>
		</div>
	</div>
	...
	<script>
		const customerEnquriesChartXValues = [
			<c:forEach items="${model_customerEnquiresDateCountEmp}" var="row"
				varStatus="status">
		...
		new Chart("customerEnquriesChartEmp", {
		  type: "line",
		  data: {
		...
		const followUpsChartXValues = [
			<c:forEach items="${model_customerFollowUpsDateCountEmp}" var="row"
				varStatus="status">
				"${row[0]}"${not status.last ? ',' : ''}
			</c:forEach>
		];
		...
		const sellCourseChartXValues = [
			<c:forEach items="${model_courseSellCountEmp}" var="row"
				varStatus="status">
				"${row[0]}"${not status.last ? ',' : ''}
			</c:forEach>
		];
